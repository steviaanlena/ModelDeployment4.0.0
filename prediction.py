...
try:
  ...
  st.info("Installing xgboost library...")
  !pip install xgboost  # This line uses magic command for Streamlit to run shell command

# Load the machine learning model and encode (assuming files are in the same directory)
try:
  model = joblib.load('best_model.pkl')
  gender_encode= joblib.load('gender_encode.pkl')
  label_encode_geo=joblib.load('label_encode_geo.pkl')
except FileNotFoundError:
  st.error("Error: Encodings or model file not found. Please ensure they are in the same directory.")
  exit(1)
